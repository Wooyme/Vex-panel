...
// 8-bit Arcade Button Click
export function playArcadeClick(freq = 600, duration = 0.04) {
...
// Joystick Step / Tick
export function playJoystickTick() {
  if (!soundEnabled) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
...
// Slider Notch Tick
export function playSliderTick(freq = 520) {
  if (!soundEnabled) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
...
// Estop Alarm Buzz
export function playEstopAlarm() {
  if (!soundEnabled) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
...
// Mode Switch Tone
export function playModeSwitchTone() {
  if (!soundEnabled) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
